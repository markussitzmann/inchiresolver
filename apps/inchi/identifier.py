...
class InChI:
	...
	def _validate(self, string, patternString):
		...
		if match:
			...
			self.element['well_formatted'] = '%s%s/%s' % (self.element['prefix'], self.element['version'], self.element['layers'])
...
class InChIKey:
	...
	def _validate(self, string, patternString):
		...
		if match:
			...
			self.element['well_formatted'] = '%s%s-%s-%s' % (self.element['prefix'], self.element['block1'], self.element['block2'], self.element['block3'])
			self.element['well_formatted_no_prefix'] = '%s-%s-%s' % (self.element['block1'], self.element['block2'], self.element['block3'])
